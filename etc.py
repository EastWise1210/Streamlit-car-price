...
경력의 의미 --> 실제 서비스 구현의 누적치
프로젝트를 많이 해놔야하는 이유는?  -->  경력을 커버할 수 있다
신입인데 나이가 많다 : 프로젝트의 질, 양으로 커버해라


지금 하고 있는 인공지능쪽 프로젝트 최소한 2개 이상 --> 5개~10개 많이 할 수록 좋다

면접관의 생각
-> 경력은 없는데 프로젝트가 많다? : 써먹을 실력은 되네 -> 어차피 실력은 비슷하니, 돈을 더 적게 주고 경력자보다 신입을 채용하는게 낫겠다
...
프로젝트 진행
프로젝트는 이력서에 필수이므로, 반드시 2개 이상 진행해야 한다.
데이터분석쪽 잘 모르는 경우는, 강사와 함께한 플젝을 보완해서 제출하기

1. 데이터 탐색 후 데이터 선택
2. 무엇을 분석할지 기획
3. 주피터노트북이나 코랩에서 분석
4. 인공지능 개발이 가능한 데이터이면 데이터 가공/학습
5. 앱 대시보드의 화면 기획
6. 개발해서 EC2에 적용
...
블로그 주제
1. AWS EC2 인스턴스 생성하기
2. 윈도우즈에서 AWS EC2 인스턴스에 접속하기 위한 방법
   -> PuTTY 설정(Session, Auth-Credential)과 AWS 설정(firewall)
3. EC2 인스턴스에 Anaconda 설치
4. EC2 인스턴스 Anaconda의 Virtual Environment 구현
5. EC2 인스턴스에 Git 설치방법, 원격 Repository와 연동하기(clone)
6. Streamlit run을 통해 해당 서비스 접근할 때 
