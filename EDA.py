            ...
            st.pyplot(fig2)
                ...
                if len(columns) != 0:
                    ...
                    if z == len(columns):
                        ...
                        if len(columns) >= 2:
                            #선택한 컬럼 전체가 문자열이거나, 선택 컬럼이 없으면 해당이 되지만 2개 이상의 컬럼 중 숫자 컬럼이 1개만 있을 때 해당사항은 없다
                            #어떻게 로직을 구성해야 -> 2개 이상의 컬럼 중 숫자 컬럼이 1개일 때도 예외 처리가 될지?
                            #경우의 수 --> 1. 선택한 컬럼이 0개일 때 <-> 1개 이상일 때
                            #             2. 1개 이상의 선택 컬럼이 모두 문자열 컬럼일 때 <-> 아닐 때
                            #             3. 1개 이상의 선택 컬럼 중 숫자열 컬럼이 1개일 때 <-> 아닐 때
                            st.dataframe(df[columns].corr(numeric_only=True))
                        elif len(columns) == 1 :
